...
1. `gather_papers()` — fetches `cs.AI`, `cs.HC`, `cs.CL`, `cs.CY` RSS feeds, deduplicates by `arxiv_id`, returns up to 30 papers
2. `call_gemini()` — sends all papers in one prompt to `gemini-2.5-flash`; parses the JSON response; retries on 503 (up to 5×, with backoff); raises `QuotaExceededError` on 429 so `main()` can switch keys
3. Render functions (`render_therm`, `render_picks`, `render_papers`, `render_llm`, `render_prompt`) — each takes the relevant slice of the parsed JSON and returns an HTML string
4. `build_html()` — reads `digest_template.html` and replaces `{{PLACEHOLDER}}` tokens with rendered HTML
5. `save_html()` — writes both `digest_YYYY-MM-DD.html` and `index.html` (GitHub Pages entry point)
...
- **Change paper counts**: edit the `rules` section of the prompt and the corresponding render functions
